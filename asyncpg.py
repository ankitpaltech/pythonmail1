...
for i in  data :
    ...
    str = mailobj['from']+mailobj['subject']+mailobj['date']+mailobj['message']
    h = hashlib.md5(json.dumps(mailobj).encode('utf-8'))
    

    
    arr.append(mailobj)

    sql = "SELECT count(*) FROM mail WHERE hash_value = '" + str(h) + "';"
    print(sql)
